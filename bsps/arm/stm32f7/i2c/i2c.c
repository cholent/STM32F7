...
#define STM32F7_I2C_INITIAL_BITRATE 100000

#define I2C_RW_BIT 0x1

stm32f7_rcc_index i2c_rcc_index [] = {
  STM32F7_RCC_I2C1,
  STM32F7_RCC_I2C2,
};

static stm32f7_rcc_index i2c_get_rcc_index(stm32f7_i2c_bus_entry *e)
{
  return i2c_rcc_index [e->index];
}

static uint32_t i2c_get_pclk(stm32f7_i2c_bus_entry *e)
{
  (void) e;

  return STM32F7_PCLK1;
}
...
)
{
  ...
  if(br <= 100000)
  {
    ...
    trise = STM32F7_I2C_TRISE_SET(trise, trise_val);

    if(ccr_val > STM32F7_I2C_CCR_CCR_MAX)
    {
      return RTEMS_INVALID_NUMBER;
    }

    /* standard mode */
    ccr &= ~STM32F7_I2C_CCR_FS;
    ccr = STM32F7_I2C_CCR_CCR_SET(ccr, ccr_val);
  }
  else
  {
...
static void stm32f7_i2c_handler(void *arg)
{
  ...
  if(read) {
    ...
    if(len == 1) {
      ...
      if(sr1 & STM32F7_I2C_SR1_ADDR) {
        cr1 = regs->cr1;
        cr1 &= ~STM32F7_I2C_CR1_ACK;
        regs->cr1 = cr1;
        ...
        cr1 = regs->cr1;
        cr1 |= STM32F7_I2C_CR1_STOP;
        regs->cr1 = cr1;
      } else if(sr1 & STM32F7_I2C_SR1_RxNE) {
        *data = regs->dr;
        wake_task = true;
      }
    } else if (len == 2) {
      ...
      if(sr1 & STM32F7_I2C_SR1_ADDR) {
        ...
        cr1 &= ~STM32F7_I2C_CR1_ACK;
        regs->cr1 = cr1;
      } else if(sr1 & STM32F7_I2C_SR1_BTF) {
        cr1 = regs->cr1;
        cr1 |= STM32F7_I2C_CR1_STOP;
        regs->cr1 = cr1;
      ...
      if(sr1 & STM32F7_I2C_SR1_ADDR) {
        /* Read sr2 to clear flag */
        regs->sr2;
      } else if(sr1 & STM32F7_I2C_SR1_BTF && data == last - 2) {
        cr1 = regs->cr1;
        cr1 &= ~STM32F7_I2C_CR1_ACK;
        regs->cr1 = cr1;
        ...
        *data = regs->dr;
        ++data;
      } else if((sr1 & STM32F7_I2C_SR1_RxNE) && (data != last - 2)) {
        *data = regs->dr;
    ...
    if((sr1 & (STM32F7_I2C_SR1_ADDR | STM32F7_I2C_SR1_TxE)) && (data <= last)) {
      regs->dr = *data;
      ++data;
    } else if(sr1 & STM32F7_I2C_SR1_BTF) {
...
static rtems_status_code i2c_wait_done(stm32f7_i2c_bus_entry *e)
{
...
rtems_status_code stm32f7_i2c_init(stm32f7_i2c_bus_entry *e)
{
  ...
  volatile stm32f7_i2c *regs = e->regs;
  stm32f7_rcc_index rcc_index = i2c_get_rcc_index(e);
  uint32_t pclk = i2c_get_pclk(e);
  uint32_t cr1 = 0;
  uint32_t cr2 = 0;
  ...
  sc = stm32f7_i2c_set_bitrate(e, STM32F7_I2C_INITIAL_BITRATE);
  RTEMS_CHECK_SC(sc, "set bitrate");

  /* Set config registers */
  cr2 = regs->cr2;
  cr2 = STM32F7_I2C_CR2_FREQ_SET(cr2, pclk / 1000000);
  cr2 |= STM32F7_I2C_CR2_ITEVTEN;
  cr2 |= STM32F7_I2C_CR2_ITBUFEN;
  regs->cr2 = cr2;

  cr1 = regs->cr1;
  cr1 |= STM32F7_I2C_CR1_PE;
  regs->cr1 = cr1;
...
)
{
  ...
  cr1 = regs->cr1;
  if(e->len == 2) {
    cr1 |= STM32F7_I2C_CR1_POS;
  } else {
    cr1 &= ~STM32F7_I2C_CR1_POS;
  }
  cr1 |= STM32F7_I2C_CR1_ACK;
  cr1 |= STM32F7_I2C_CR1_START;
  regs->cr1 = cr1;
